                ...
                <div className={"grid grid-cols-2 gap-x-8 p-4"}>
                    {partners.map((partner) => (
                        <a key={partner.name} href={partner.website}
                           className={"relative aspect-square max-h-72 bg-white mx-auto w-3/4 rounded-3xl flex items-center justify-center m-4 p-8 border-4 hover:border-red-500 hover:scale-125 duration-300 ease-in-out"}>
                            <Image src={"/sponsors/" + partner.logo} alt={partner.name} fill={true} style={{objectFit: 'contain'}}/>
                        </a>
                    ))}
                </div>
            </div>
            ...
            <div id={"silver"}>
                ...
                <div className={"flex flex-row overflow-auto gap-x-8"}>
                    {silverSponsors.map((partner) => (
                        <a key={partner.name} href={partner.website}
                           className={"relative h-40 bg-white mx-auto w-3/4 rounded-3xl flex items-center justify-center m-4 p-4 border-4 hover:border-gray-400 hover:scale-110 duration-300 ease-in-out"}>
                            <Image src={"/sponsors/" + partner.logo} alt={partner.name} width={200} height={300}/>
                        </a>
                    ))}
                </div>
