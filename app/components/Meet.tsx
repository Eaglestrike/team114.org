        ...
        <div className={"bg-slate-900 p-y-10"}>
            ...
            <div className={"w-screen grid grid-cols-4 gap-3 p-4 justify-items-center"}>
                ...
                {captains.map((captain, index) => (
                    <div className={"bg-slate-600 rounded-3xl justify-items-center p-4"}>
                        <div className={'h-fit w-fit lg:h-52 sm:h-20 aspect-square relative'}>
                            <Image fill={true} src={captain.image} alt={"captain"}
                                   className={"object-contain aspect-square"} quality={100}/>
                        </div>
            ...
            <div className={"w-screen grid grid-cols-4 gap-x-6 gap-y-8 justify-items-center pb-10"}>
                {leads.map((lead, index) => (
                    <div className={"bg-slate-500 rounded-3xl justify-items-center p-4"}>
                        <div className={'h-fit w-fit lg:h-52 sm:h-20 aspect-square relative'}>
                            <Image fill={true} src={lead.image} alt={lead.role}
                                   className={"object-contain aspect-square"} quality={100}/>
                        </div>
                        <div className={'bg-gray-20 text-center text-white text-1xl m-1'}>{lead.role}:</div>
                        <div className={'bg-gray-20 text-center text-white text-2xl'}>{lead.name}</div>
                    </div>
                ))}
            </div>
        </div>
